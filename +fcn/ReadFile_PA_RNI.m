function Data_PA_RNI = ReadFile_PA_RNI(app)
            ...
            % Especifica o nome do arquivo CSV com os cálculos que será criado
            filename = '\DataBase\PA_RNI\Dados_PA_RNI_Calculados.xlsx';

            % Obter o caminho absoluto do Arquivo de saida gerado
            Path_Data_PA_RNI_Out = fullfile(DirApp, filename);
            
            % Verificar se o arquivo de saída existe no diretório atual
            if ~isfile(Path_Data_PA_RNI_Out)
                % Escreve a tabela no arquivo XLSX
